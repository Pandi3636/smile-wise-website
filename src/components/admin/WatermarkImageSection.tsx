...
type WatermarkSettings = {
  ...
  size: number;
  color: string;
  x?: number; // Horizontal position for "custom"
  y?: number; // Vertical position for "custom"
...
switch (watermarkSettings.position) {
  ...
  case "center":
    ...
    break;
  case "custom":
    x = watermarkSettings.x ?? 0;
    y = watermarkSettings.y ?? 0;
    break;
            ...
            <div className="space-y-2">
              ...
              {watermarkSettings.position === 'custom' && (
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <Label htmlFor="custom-x">X (horizontal)</Label>
                    <Input
                      id="custom-x"
                      type="number"
                      value={watermarkSettings.x ?? 0}
                      onChange={(e) => updateWatermarkSetting('x', parseInt(e.target.value))}
                      placeholder="e.g. 100"
                    />
                  </div>
                  <div>
                    <Label htmlFor="custom-y">Y (vertical)</Label>
                    <Input
                      id="custom-y"
                      type="number"
                      value={watermarkSettings.y ?? 0}
                      onChange={(e) => updateWatermarkSetting('y', parseInt(e.target.value))}
                      placeholder="e.g. 100"
                    />
                  </div>
                </div>
